...
#
echo "make_rrfs_wind.sh fetching rrfs : time = ${1}, {cyc}le = ${2}, domain = ${3}"
...
for file in $(ls "${INPUT_DIR}"/rrfs.t${2}z.2dfld.*km.f*.${3}.grib2 | sort); do
...
echo "rrfs processing complete for forecast date ${1}, {cyc}le ${2}, domain ${3}"
