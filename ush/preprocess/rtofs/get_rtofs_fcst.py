...
with nc.Dataset(flout, 'w', format='NETCDF4') as ncout:
    ...
    MT_var=ncout.createVariable("MT", 'f8', ('T',))
    var=data["MT"]
    iutil.CopyAttributes(var, MT_var)
    MT_var[:] = MT[:]

    u_var               = ncout.createVariable('u-vel', 'f4', ('T','Layer','X','Y'),fill_value    = fill_value0)
    var=data["u_velocity"]
    iutil.CopyAttributes(var, u_var)
    u_var[:,:,:,:]      = U

    v_var               = ncout.createVariable('v-vel', 'f4', ('T','Layer','X','Y'),fill_value    = fill_value0)
    var=data["v_velocity"]
